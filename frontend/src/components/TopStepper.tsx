...
const TopStepper: React.FC<{ activeStep?: number }> = ({ activeStep = 1 }) => {
  return (
      ...
      {steps.map((step, index) => (
        <Box key={step.num} sx={{ display: "flex", alignItems: "center" }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 0.8, flexShrink: 0 }}>
            {step.done ? (
              ...
              <Box sx={{
                ...
                bgcolor: step.active ? "#1565c0" : "#e2e8f0", color: step.active ? "#fff" : "#94a3b8", fontSize: 11, fontWeight: 700
              }}>{step.num}</Box>
            )}
            <Box>
          ...
          {index < steps.length - 1 && (
            <Box sx={{ width: 20, height: 1, bgcolor: "#e2e8f0", mx: 1, flexShrink: 0 }} />
          )}
        </Box>
      ))}
    </Box>
